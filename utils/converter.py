...
async def extract_audio_ffmpeg(input_mp4: str, output_mp3: str) -> bool:
    ...
    try:
        ...
        if proc.returncode == 0 and os.path.exists(output_mp3) and os.path.getsize(output_mp3) > 0:
            return True
        ...
        if proc_f.returncode == 0 and os.path.exists(output_mp3) and os.path.getsize(output_mp3) > 0:
            return True
...
async def convert_video_note_ffmpeg(input_mp4: str, output_note_mp4: str) -> bool:
    ...
    # Primary filter: crop square from center then scale to 640x640
    ...
    try:
        ...
        if proc.returncode == 0 and os.path.exists(output_note_mp4) and os.path.getsize(output_note_mp4) > 0:
            return True

        logger.warning(f"FFmpeg primary video note conversion failed (code {proc.returncode}): {stderr.decode(errors='ignore')}. Trying fallback...")
        
        # Fallback filter: scale to cover then crop 640x640
        cmd_fallback = [
            ffmpeg_exe,
            "-y",
            "-i", input_mp4,
        ...
        if proc_f.returncode == 0 and os.path.exists(output_note_mp4) and os.path.getsize(output_note_mp4) > 0:
            return True

        logger.error(f"FFmpeg video note fallback failed (code {proc_f.returncode}): {stderr_f.decode(errors='ignore')}")
        return False
    except Exception as e:
